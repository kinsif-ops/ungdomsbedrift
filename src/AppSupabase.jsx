  ...
  return (
    <>
      ...
      {profile.role === 'teacher'
        ? <TeacherDashboard profile={profile} onLogout={logout} />
        : <StudentApp profile={profile} onLogout={logout} />}
    </>
  )
}
...
function StudentApp({ profile, onLogout }) {
  ...
  async function saveName() {
    ...
    if (!n || n === company.name) { setEditingName(false); return }
    setSavingName(true)
    try {
      await db.updateCompanyName(company.id, n)
      setCompany(prev => ({ ...prev, name: n }))
      setEditingName(false)
    } catch (e) {
      alert('Kunne ikke lagre navnet: ' + (e.message || e))
  ...
  return (
    <div style={S.appRoot}>
      ...
      <header style={S.header}>
        <div>
          <button onClick={() => { setNameDraft(company.name); setEditingName(true) }}
            title="Trykk for å endre navnet"
            style={{ ...S.appTitle, background: 'none', border: 'none', padding: 0, cursor: 'pointer', fontFamily: 'inherit', textAlign: 'left' }}>
            {company.name} <span style={{ fontSize: 12, opacity: 0.35 }}>✎</span>
          </button>
      ...
      {editingName && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.45)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 200, padding: 18 }}>
          <div style={{ background: '#fff', borderRadius: 18, padding: '24px 22px', width: '100%', maxWidth: 400 }}>
            <h2 style={{ fontSize: 17, fontWeight: 800, color: '#1e293b', margin: '0 0 6px' }}>Navn på bedriften</h2>
            <p style={{ fontSize: 12, color: '#64748b', lineHeight: 1.6, margin: '0 0 14px' }}>
              Er dere registrert hos Ungt Entreprenørskap, bør navnet her stemme med det som står i registreringen.
            </p>
            <input style={S.input} value={nameDraft} autoFocus maxLength={60}
              onChange={e => setNameDraft(e.target.value)}
              onKeyDown={e => { if (e.key === 'Enter') { e.preventDefault(); saveName() } }} />
            <div style={{ display: 'flex', gap: 8, marginTop: 14 }}>
              <button onClick={() => setEditingName(false)}
                style={{ flex: 1, padding: '11px', borderRadius: 12, border: '1.5px solid #e2e8f0', background: '#f8fafc', cursor: 'pointer', fontFamily: 'inherit', fontWeight: 600, color: '#64748b' }}>Avbryt</button>
              <button onClick={saveName} disabled={!nameDraft.trim() || savingName}
                style={{ ...S.btnPrimary, flex: 2, marginTop: 0, opacity: nameDraft.trim() ? 1 : 0.5 }}>
...
function TeacherDashboard({ profile, onLogout }) {
  ...
  const [moving, setMoving] = useState(null)   // { user, from } – elev som skal flyttes

  async function doMove(toCompanyId) {
    if (!moving) return
    const target = companies.find(c => c.id === toCompanyId)
    const label = target ? target.name : 'ingen bedrift'
    if (!window.confirm(
      `Flytte ${moving.user.name} fra ${moving.from.name} til ${label}?\n\n` +
      `Oppgaver eleven er tilordnet i ${moving.from.name} frigjøres, og kunder ` +
      `eleven står ansvarlig for mister ansvarlig. Selve arbeidet blir værende i bedriften.`
    )) return
    try {
      await db.moveStudent({ userId: moving.user.id, fromCompanyId: moving.from.id, toCompanyId })
      setMoving(null)
      const data = await db.getCompaniesForTeacher(profile.school)
      setCompanies(data)
    } catch (e) {
      alert('Kunne ikke flytte eleven: ' + (e.message || e))
    }
  }
  const [loading, setLoading] = useState(true)
  ...
  return (
    <div style={S.appRoot}>
      ...
      {/* Flytt elev mellom bedrifter */}
      {moving && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.45)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 200, padding: 18 }}>
          <div style={{ background: '#fff', borderRadius: 18, padding: '24px 22px', width: '100%', maxWidth: 420, maxHeight: '85vh', overflowY: 'auto' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 4 }}>
              <Avatar name={moving.user.name} size={34} />
              <div>
                <div style={{ fontSize: 16, fontWeight: 800, color: '#1e293b' }}>{moving.user.name}</div>
                <div style={{ fontSize: 11, color: '#94a3b8' }}>Nå i {moving.from.name}</div>
              </div>
            </div>
            <p style={{ fontSize: 12, color: '#64748b', lineHeight: 1.6, margin: '12px 0 14px' }}>
              Velg hvilken bedrift eleven skal flyttes til. Arbeidet som allerede er gjort
              blir værende i den gamle bedriften – det er bare eleven som flyttes.
            </p>

            <div style={{ display: 'flex', flexDirection: 'column', gap: 7 }}>
              {companies.filter(c => c.id !== moving.from.id).map(c => (
                <button key={c.id} onClick={() => doMove(c.id)}
                  style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '11px 13px', borderRadius: 12, border: '1.5px solid #e2e8f0', background: '#f8fafc', cursor: 'pointer', fontFamily: 'inherit', textAlign: 'left' }}>
                  <span style={{ fontSize: 18 }}>🏢</span>
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: 13, fontWeight: 700, color: '#1e293b' }}>{c.name}</div>
                    <div style={{ fontSize: 11, color: '#94a3b8' }}>{(c.company_members || []).length} elever</div>
                  </div>
              ...
              {companies.length < 2 && (
                <div style={{ fontSize: 12, color: '#94a3b8', fontStyle: 'italic', padding: '8px 0' }}>
                  Det finnes ingen andre bedrifter å flytte til ennå.
                </div>
              )}
            </div>
            ...
            <div style={{ marginTop: 14, paddingTop: 14, borderTop: '1px dashed #e2e8f0' }}>
              <button onClick={() => doMove(null)}
                style={{ width: '100%', padding: '10px', borderRadius: 12, border: '1.5px solid #fecaca', background: '#fef2f2', color: '#dc2626', fontSize: 12, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}>
                Ta ut av bedriften (uten ny)
              </button>
              <p style={{ fontSize: 11, color: '#94a3b8', lineHeight: 1.5, margin: '7px 0 0' }}>
                Eleven beholder kontoen sin, men står uten bedrift og kan bli med i en ny med kode.
              </p>
            </div>

            <button onClick={() => setMoving(null)}
              style={{ width: '100%', marginTop: 14, padding: '11px', borderRadius: 12, border: '1.5px solid #e2e8f0', background: '#f8fafc', cursor: 'pointer', fontFamily: 'inherit', fontWeight: 600, color: '#64748b' }}>
              Avbryt
            </button>
          </div>
        </div>
        ...
        {!selected ? (
          ...
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginBottom: 4 }}>
              {(selected.company_members || []).map(m => {
                const u = m.profiles
                if (!u) return null
                return (
                  <button key={m.user_id} onClick={() => setMoving({ user: u, from: selected })}
                    title="Flytt eleven til en annen bedrift"
                    style={{ ...S.memberChip, border: '1px solid #e2e8f0', cursor: 'pointer', fontFamily: 'inherit' }}>
